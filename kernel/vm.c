...
{
  kernel_pagetable = kama_kvminit_newpgtbl(); // 全局内核页表仍然使用kvminit进行初始化操作
...
void
kvmmap(pagetable_t pgtbl, uint64 va, uint64 pa, uint64 sz, int perm)  //va : virtual address, pa physical address  kvmmap 将某个虚拟地址映射到某个物理地址
{
...
uint64
kvmpa(pagetable_t pgtbl,uint64 va)   //将虚拟地址翻译为物理地址
{
...
pagetable_t
kama_kvminit_newpgtbl()   //创建一个页表并初始化，并返回这个页表
{
